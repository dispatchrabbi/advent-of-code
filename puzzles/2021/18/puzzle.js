...
async function part1(input) {
  ...
  for(let i = 1; i < snumbers.length; ++i) {
    // log.debug('   ' + runningSnotal.format());
    // log.debug(' + ' + snumbers[i].format());
    // log.debug(' = ' + snadd(runningSnotal, snumbers[i]).format());
    // log.debug('\n');
