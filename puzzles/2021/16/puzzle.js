...
function decodePacket(packet) {
  ...
  // log.debug(`Attempting to decode packet (type ${type}):`);
  console.group();
...
function decodeLiteralPacket(packet) {
  // log.debug(`Decoding literal packet (size ${packet.length * 8})...`);
...
function decodeOperatorPacket(packet) {
  // log.debug(`Decoding operator packet (size ${packet.length * 8})...`);
  // log.debug(`packet starts with: ${readBits(packet, 0, 7).toString(2)}`);
  let cursor = 6; // skipping the version and type fields
  const subpackets = [];
  ...
  if(mode === MODES.TOTAL_BIT_LENGTH) {
    // log.debug(`...total bit length mode...`);
    const totalBitLength = readBits(packet, cursor, 15);
    cursor += 15;

    // log.debug(`bitlength expected: ${totalBitLength}`);
    ...
    // log.debug(`...number of subpackets mode...`);
    const numberOfSubpacketsExpected = readBits(packet, cursor, 11);
    cursor += 11;

    // log.debug(`subpackets expected: ${numberOfSubpacketsExpected}`);
...
function printPacket(packet, cursor = 0) {
  log.debug(`PACKET v.${packet.version} (cursor: ${cursor}, len: ${packet.length})`);
  log.debug(`type: ${REVERSE_PACKET_TYPES[packet.type]} (${packet.subpackets ? packet.subpackets.map(subpacket => subpacket.value).join(' ') : packet.value})`);
  log.debug(`result: ${packet.value}`);

  cursor += packet.length - (packet.subpackets ? packet.subpackets.reduce((total, subpacket) => total + subpacket.length, 0) : 0);

  if(packet.subpackets) {
    log.debug(`subpackets:`);
    console.group();
